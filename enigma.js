...
function Enigma_code(txt,orientation,rotor,plug_board){
    ...
    for(let i=0;i<txt.length;i++){
        ...
        if(txt[i]!=' '){
            ...
            let s_input= plug_board[to_ascii(txt[i])]//console.log("s_input",s_input)
            ...
            let charval=String.fromCharCode(65+ plug_board[r3])
...
function plug_board(s){
    a=[]
    for(let i=0;i<26;i++){a.push(i)}
    console.log(a)
    for(let i=0;i<s.length;i++){
        let ac=s[i];
        a[to_ascii(ac[0])]=to_ascii(ac[1]);
        a[to_ascii(ac[1])]=to_ascii(ac[0]);
    }
    console.log(a)
    return a;
}
...
function takeinput(){       
    ...
    o=o.map(to_int);
    r=r.map(to_ascii)
    let alphabet_comb=[];for(let i=0;i<26;i++){alphabet_comb.push(i)}
    ...
    if(p.length!=1){alphabet_comb=plug_board(p)}
    ...
    let ans=Enigma_code(t,o,r,alphabet_comb);
